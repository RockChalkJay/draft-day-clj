...
- **Per-league draft state.** `:teams`, `:drafted`, `:picks` and `:my-team-id`
  ...
  `fx/storage-version` bump when it happens. It reaches the phase too:
  `db/derived-phase` counts a draft that filled every roster as the season
  having started, so finishing one league's auction in August opens every
  automatic league in season until week 1 is played.
