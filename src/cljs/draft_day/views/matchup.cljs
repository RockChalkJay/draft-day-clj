  ...
  (let [m      @(rf/subscribe [:matchup])
    ...
    [:div.mu-strip
     [:span
      ...
      ;; Dated for `waivers/week-note`'s reason: it revises through the week as
      ;; inactives land, and a board that cannot date itself implies it is
      ;; current.
...
(defn rows
  ...
  (let [seat-rows (fn [ls rs seat? tag]
    ...
    [:<>
     ...
     ;; Child order mirrors `player-cell` exactly. It did not, once, and the
     ;; totals row read actual-then-projected under nine seats reading
     ;; projected-then-actual — two columns of numbers that did not line up with
     ;; the column they were totalling.
     [:div.mu-tot
...
(defn matchup-view []
  (let [m      @(rf/subscribe [:matchup])
    ...
    [:div.mu-view
     [week-strip]
     (cond
       ;; Four states, not two. Which one it is decides what the manager should
       ;; do next, and collapsing them would leave him guessing.
