...
(def DROP-THRESHOLD
  ...
  now cuts a pool-sized number of tiers at its biggest gaps rather than
  thresholding each drop. The two still share `tiers/relative-drop` so 'a fall of
  x%' means one thing on the board; only tcm asks the threshold question, and it
  asks it across two roster slots — 'if I pass, what is still here when it is my
  turn again?' — so the same steepness shows up as a bigger number than a
