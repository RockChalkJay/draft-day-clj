...
(deftest a-team-with-no-player-ids-is-repaired-not-trusted
  ...
  (let [out (db/reconcile-league-sync {:provider "sleeper"
    ...
    (testing "but a lineup is repaired by position, not by membership"
      ;; Dropping a nil there slides every seat below it up one, which the seat
      ;; labels have no way to notice — see `db/repair-lineup`.
...
;; Sorting on `:week/opponent` alone clumped every ESPN-answered row at one end
;; of the column while the cell visibly showed an opponent.
