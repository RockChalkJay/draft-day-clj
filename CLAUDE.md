...
- `static-rankings`: `model` (stat line -> `:points`, via `rankings/model.clj`) -> `projections` (floor/ceiling band from expert-rank disagreement) -> `replacement` (replacement level) -> VORP -> `tiers` (gap detection above replacement) -> `injury` (the `:injury-risk` display signal)
