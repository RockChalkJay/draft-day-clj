...
- `static-rankings`: `model` (stat line -> `:points`, via `rankings/model.clj`) -> `projections` (floor/ceiling band from expert-rank disagreement) -> `replacement` (replacement level) -> `tiers` (cliff detection per position, above replacement) -> VORP
...
Tiers are cut where the drop to the next player exceeds `tiers/DROP-THRESHOLD` — a *relative* drop, the same shape `tcm` uses (their constants differ because tcm looks two roster slots ahead, and both docstrings say so). How many tiers a position gets follows its own cliff structure, so there is no tier-count setting; everything at or below replacement shares a final tail tier, and K/DST get a floor from `engine/tier-floor` since they are absent from the replacement map. `:fantasypros/ecr-tier` no longer overrides the computed tier — it is PPR-only and covered most of the board, which made Tier the one column that ignored the league's scoring; it now rides along for the opt-in `:fp-tier` column.
